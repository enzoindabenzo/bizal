...
# Enforce valid status transitions to prevent logically invalid
...
#
# Co-locate the valid-status set here so it is derived from the
...
class BookingListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Use get_effective_role() instead of getattr(user, 'role', '').
    ...
    def create(self, request, *args, **kwargs):
        # Explicitly reject requests with no resolved tenant (e.g. a
    ...
    def perform_create(self, serializer):
        ...
        if booking.booking_type == 'room_booking' and booking.resource_type == 'room' and booking.resource_id:
            try:
                ...
                # Unexpected failure (DB error, IntegrityError, schema change) must be logged.
...
class BookingDetailView(generics.RetrieveUpdateAPIView):
    ...
    def get_queryset(self):
        ...
        # Use get_effective_role() so deactivated staff (is_active=False
...
def cancel_booking(request, pk):
    # Use get_effective_role() so deactivated staff cannot cancel
    ...
    # Include 'updated_at' explicitly — Django 4.2 injects auto_now
...
def admin_update_booking(request, pk):
    ...
    if new_status:
        ...
        # Enforce the transition map. Terminal states (completed,

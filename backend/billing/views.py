...
except ImportError:  # pragma: no cover - xhtml2pdf is always installed in this environment
    pisa = None
...
class LoyaltyMeView(APIView):
    ...
    def get(self, request):
        ...
        return Response(LoyaltyAccountSerializer(account, context={'request': request}).data)
...
@api_view(['GET'])
# Invoice_pdf was only guarded by IsTenantOwner. InvoiceListCreateView
  ...
  {f'<p>{esc(invoice.notes)}</p>' if invoice.notes else ''}
    ...
    try:
        if pisa is None:
            raise ImportError
        result = BytesIO()
        pdf_status = pisa.CreatePDF(html, dest=result) # check for render errors
        if pdf_status.err:
        ...
        # Sanitise invoice_number before embedding in Content-Disposition.

...
def award_points(tenant, user, amount_spent, *, reason, source_type, source_id):
    ...
    if not user or not getattr(user, 'is_authenticated', False):
        return None
    try:
        # Use Decimal arithmetic instead of float() to avoid precision
    ...
    # Wrap get_or_create in atomic() so Django's internal
    ...
    #
    # Catch IntegrityError from the UniqueConstraint on LoyaltyTransaction

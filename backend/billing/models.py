...
class LoyaltyAccount(TenantScopedUUIDModel):
    ...
    def add_points(self, amount, reason, source_type='', source_id=''):
        ...
        use select_for_update() + F() expressions inside
        ...
        Raises ValueError if a negative amount would take the balance below
        zero, rather than silently clamping the balance to 0 while still
        recording the full unclamped amount in the ledger — that mismatch
        (balance says 0, ledger says -500) would make the transaction
        history lie about what actually happened. No caller currently
        passes a negative amount, but this guards the method itself for
        when redemptions are added.
        """
        if amount == 0:
            return
        ...
        with transaction.atomic():
            ...
            if amount < 0 and account.points + amount < 0:
                raise ValueError(
                    f"Cannot redeem {-amount} points: balance is only {account.points}."
                )
...
class LoyaltyTransaction(TenantScopedUUIDModel):
    ...
    class Meta:
        ordering = ['-created_at']
        constraints = [
            # Prevent double-award race. Two concurrent webhook
...
class InvoiceLine(TenantScopedUUIDModel):
    ...
    class Meta:
        # LOW-severity fix: without this, invoice.lines.all() (used by
        # InvoiceSerializer's nested `lines` field) has no guaranteed order.
        # Not a pagination data-loss bug like RoomBooking (this is never
        # paginated — the full set is always returned), but line items on
        # an invoice could still visibly reshuffle between page loads,
        # which looks broken to a tenant reviewing a bill.
        ordering = ['created_at']
    ...
    def save(self, *args, **kwargs):
        # Previously this called super().save() then

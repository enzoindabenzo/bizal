...
# RAILWAY / NO-NGINX FIX: Django's static() helper above is a silent no-op
# whenever DEBUG=False — it returns [] instead of registering the route,
# because Django expects a real web server (nginx in docker-compose.prod.yml)
# to serve MEDIA_URL in production instead. Railway has no nginx sitting in
# front of gunicorn, so without this, every uploaded file (avatars, product
# images, etc.) would 404 in production even though the file exists on disk.
# This re-registers /media/<path> using Django's own file-serving view,
# gated on an explicit env var (not DEBUG) so it stays OFF by default for
# anyone who *does* run nginx/S3 in front, and is opt-in only for single-
# container hosts like Railway. Uses django.views.static.serve directly
# instead of static() because static() hardcodes the DEBUG check internally
# and can't be overridden by passing DEBUG=True.
if os.environ.get('SERVE_MEDIA_WITHOUT_NGINX', 'False').lower() in ('1', 'true', 'yes'):
    urlpatterns += [
        re_path(
            r'^media/(?P<path>.*)$',
            serve_media,

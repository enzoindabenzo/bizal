...
Management command to re-seed django-celery-beat PeriodicTask entries
...
# Celery's crontab() parses each field (minute/hour/day_of_week/
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for name, config in schedule_conf.items():
            ...
            # When Celery is not installed, base.py replaces
            ...
            if isinstance(sched, crontab):
                # _orig_* attributes are private/undocumented Celery
                ...
                #
                # The fallback values (sched.minute, sched.hour, ...) are
                ...
                # Include timezone='UTC' in the lookup key, not in
                ...
                # Wrap in atomic + advisory lock to prevent duplicate
                ...
                # Use a pg_advisory_xact_lock so only one
                # web container at a time can run the get_or_create for crontab schedules.
                # Skip the DB write entirely in dry-run mode so --dry-run
                ...
                # Wrap in the same transaction.atomic() guard
                ...
                # duplicate rows.
                # Guard the IntervalSchedule DB write with dry_run,

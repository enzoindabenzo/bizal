...
# Also reject the docker-compose dev placeholder so operators who
...
# Allow SECURE_SSL_REDIRECT to be disabled via env var until TLS is
...
# Only emit HSTS headers (and submit to preload lists) when TLS is
...
# Use top-level ImproperlyConfigured import (already imported at top of file)
...
# Docker-compose.prod.yml health checks hit
...
# Strip any trailing slash so URL construction never produces
# double-slash paths (e.g. https://bizal.al//verify-email/...).
_frontend_url = _frontend_url.rstrip('/')
# Assign the stripped value back to the Django setting so that
...
# FRONTEND_BASE_URL must be the main domain (bizal.al), never a
...
# The original check (`_frontend_host != 'bizal.al'`) was too strict —
...
# Override CORS_ALLOWED_ORIGIN_REGEXES to drop the http://
...
# This also serves as the explicit CORS documentation for production.

...
# Strip whitespace from each entry. str.split(',') does not strip
...
# ── MAIN_DOMAIN ────────────────────────────────────────────────────────────
# Used by tenants/middleware.py to distinguish the main platform host from a
# tenant subdomain (host == MAIN_DOMAIN → main site; host.endswith('.'+MAIN_DOMAIN)
# → tenant slug extracted from the subdomain). Previously hardcoded as
# 'bizal.al' directly in middleware.py, which silently broke tenant
# resolution on any deployment using a different domain (e.g. a free
# wildcard-DNS domain like <ip>.sslip.io for a pilot deployment). Now
# env-configurable like ALLOWED_HOSTS — set MAIN_DOMAIN to match whatever
...
AUTH_PASSWORD_VALIDATORS = [
    ...
    {'NAME': 'bizal.validators.FriendlyCommonPasswordValidator'},
...
# Use the modern STORAGES dict (Django 4.2+) instead of the legacy
...
# Result backend uses db/2 (kept separate from the broker's db/0 —
# FLUSHDB for cache debugging would wipe task queues, and the
...
try:
    from celery.schedules import crontab  # no-op callable so CELERY_BEAT_SCHEDULE
except ImportError:                        # constructs without TypeError on a Celery-less image
    def crontab(*args, **kwargs):          # type: ignore[misc]
        return None

# Periodic tasks (celery-beat)
# NOTE: celery-beat uses DatabaseScheduler (see docker-compose.yml).
...
#
# NOTE: sync_celery_schedule runs via the `web` container's entrypoint.sh
...
CELERY_BEAT_SCHEDULE = {
    # All hours below are UTC (stored as timezone='UTC' in
...
# Wrap the EMAIL_PORT int cast in a try/except so a misconfigured
...
# Read TLS/SSL mode from environment so operators using port 465
...
# Fail fast at startup if both TLS and SSL are True.
...
# Do NOT call sentry_sdk.init() here. Initialising Sentry in
...
# The previous pattern r'^https?://.*\.bizal\.al$' required a literal
...
# CORS_ALLOW_CREDENTIALS must be set in base.py so it is
...
UNFOLD = {
    ...
    'COLORS': {
        ...
        # Indigo — deliberately distinct from the storefront's warm parchment/
        # gold brand.css palette. This is the platform-ops surface (only
        # BizAL staff see it), so it reads as its own "internal tool" rather
        # than mirroring tenant-facing branding.
        'primary': {

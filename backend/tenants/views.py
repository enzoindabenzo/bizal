...
class TenantInfoView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        ...
        if slug:
            try:
                # Previously returned full public tenant info for
                ...
                pass
        # Log when no tenant can be resolved so frontend bugs
...
class TenantMeView(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        # Capture pre-save state to detect the False→True transition below —
        # serializer.instance is the same object DRF loaded via get_object(),
        # so this reads the value from before this PATCH is applied.
        was_incomplete = not serializer.instance.onboarding_complete

        serializer.save()
        cache.delete(f'tenant:{serializer.instance.slug}')

        # Pilot/thesis instrumentation: the frontend's finishOnboarding()
        # (onboarding.html) PATCHes {onboarding_complete: True} as its very
        # last step, regardless of plan — unlike hero-slide creation or
        # other Pro+-gated actions, this fires for every tenant on every
        # plan, so it's the one universal "onboarding finished" signal.
        # Logging it here (rather than adding a new endpoint) means every
        # real signup — including friends trying the pilot deployment —
        # gets its onboarding duration recorded automatically, with no
        # manual stopwatch needed. See research/onboarding_timing_methodology.md.
        if was_incomplete and serializer.instance.onboarding_complete:
            from activity.utils import log_activity
            tenant = serializer.instance
            duration_seconds = (timezone.now() - tenant.created_at).total_seconds()
            log_activity(
                tenant=tenant,
                actor=self.request.user,
                verb='onboarding.completed',
                description=f'{tenant.name}: onboarding completed',
                target_type='tenant',
...
def change_plan(request):
    ...
    Direct plan changes are allowed only for tenants WITHOUT an active
    Stripe subscription (see the `stripe_subscription_id` check below).
    Stripe-backed tenants are rejected here with 409 CONFLICT and must
    change plans through the billing portal instead, so Stripe stays the
    single source of truth for billing on those accounts.
    """
    ...
    # If the tenant has a live Stripe subscription, Stripe is the source of
    # truth for billing. Changing `plan` directly here would leave the DB
    # saying e.g. "starter" while Stripe keeps charging for "pro" — a real
    # billing-integrity bug. Send them through the billing portal instead,
    # where the change flows through Stripe (and the webhook) properly.
    if tenant.stripe_subscription_id:
        return Response(
            {'detail': 'You have an active subscription. Please change your plan from the billing portal.'},
...
def tenant_signup(request):
    ...
    return Response({
        ...
        'status': 'pending_activation', # signal to frontend that tokens are not immediately usable
        'slug': tenant.slug,
...
def check_slug(request):
    ...
    which leaks which business names are registered. The limit is kept tight
    (10/m) to make systematic enumeration across IP rotation meaningfully
    slower without impacting real users (onboarding wizard checks slug
    on-type, rarely more than ~5/minute).

    Also rejects reserved slugs (admin, api, health, etc.).
...
def credit_ledger(request):
    ...
    qs = CreditLedger.objects.filter(tenant=tenant).order_by('-created_at') #

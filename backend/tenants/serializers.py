...
class TenantPublicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            # Reply_from_email removed — it's an internal sending address
            ...
            # 'plan', 'trial_days_remaining', and 'trial_expired' removed.
            ...
            # 'onboarding_step' and 'onboarding_complete' removed.
...
class TenantSignupSerializer(serializers.Serializer):
    ...
    # Single source of truth — use Tenant._RESERVED_SLUGS directly
    ...
    def validate_owner_email(self, value):
        # Normalize to lowercase so the duplicate-email check matches
        # regardless of case, and the stored user is created with a consistent
        # email that will always match on login.
        value = value.strip().lower()
        # NOTE: no uniqueness check here. Per the per-tenant email policy
        # (see accounts.models.User's NOTE on `email`), the same person can
        # own accounts on multiple tenants with the same email — and since
        # this serializer always creates a brand-new tenant, there is no
        # existing tenant scope to collide with. A prior version of this
        # check rejected signup whenever the email existed *anywhere on the
        # platform*, which incorrectly blocked someone who already owns one
        # BizAL business from registering a second one with the same email.
        return value

    def validate_owner_password(self, value):
        # Enforce AUTH_PASSWORD_VALIDATORS for the tenant owner password.
...
class MarketplaceTenantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            # 'plan' removed. This is a public AllowAny endpoint — exposing
            ...
            # TenantPublicSerializer (). Plan data is available only to the

...
def expire_trials():
    ...
    now = timezone.now()
    # Prefetch each tenant's owner in the same round trip
    ...
    for tenant in expired:
        Tenant.objects.filter(pk=tenant.pk).update(is_active=False, trial_warning_sent_at=None) # clear warning flag so re-activated tenants receive new trial warning
...
def send_trial_warning_emails():
    ...
    tenants = Tenant.objects.filter(
        ...
        # Avoid N+1 owner lookups in the loop below.
        Prefetch('users', queryset=User.objects.filter(role='owner'), to_attr='_prefetched_owners')
    )

    count = 0
    for tenant in tenants:
        # Claim the send BEFORE emailing, not after: an atomic conditional
        # UPDATE here means only one worker can ever win the race, so two
        # concurrent Celery workers can no longer both pass the isnull=True
        # filter above and both send. If the claim fails (another worker
        # already claimed it), skip — no email, no double-send.
        claimed = Tenant.objects.filter(
            pk=tenant.pk, trial_warning_sent_at__isnull=True
        ).update(trial_warning_sent_at=now)
        if not claimed:
            continue
        days_left = max(0, math.ceil((tenant.trial_ends_at - now).total_seconds() / 86400)) # use math.ceil instead of double-negation trick; avoids float boundary off-by-one
        owner = tenant._prefetched_owners[0] if tenant._prefetched_owners else None
        try:
            _send_trial_warning_email(tenant, days_left, owner=owner)
        except Exception:
            # Send failed — release the claim so tomorrow's run retries this
            # tenant instead of silently losing the warning forever. Log and
            # continue rather than raising, so one bad send doesn't abort
            # the warning run for every other tenant in the batch.
            Tenant.objects.filter(pk=tenant.pk).update(trial_warning_sent_at=None)
            _tasks_logger.exception('Failed to send trial warning email for tenant %s', tenant.pk)
            continue
        count += 1
...
def _send_trial_expired_email(tenant, owner=None):
    ...
    if not owner:
        return
    # Skip anonymised owner accounts. MeDeleteView sets
    ...
    try:
        send_mail(
            ...
            # Fail_silently=False so SMTP errors raise into the except
...
def _send_trial_warning_email(tenant, days_left, owner=None):
    ...
    if not owner:
        return
    # Same anonymised-owner guard as _send_trial_expired_email.
    if not owner.is_active or not owner.email or 'deleted.bizal.al' in owner.email:
        return
    try:
        send_mail(
            ...
            from_email=settings.DEFAULT_FROM_EMAIL,
            recipient_list=[owner.email],
            # Fail_silently=False — same rationale as _send_trial_expired_email.
            fail_silently=False,
        )
    except Exception as exc:
        _tasks_logger.error(
...
def apply_referral_credits_for_active_tenants(self):
    ...
    count = 0
    for ref in pending:
        # The outer transaction.atomic() + select_for_update() that
        # previously lived here was the original concurrency guard. It became
        # redundant when apply_credit() was made self-locking:
        # apply_credit() now acquires its own transaction.atomic() +

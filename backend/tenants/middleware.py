...
# MAIN_DOMAIN is read live from settings.MAIN_DOMAIN (see settings/base.py)
# rather than frozen as a module-level constant, so it stays correct if a
# deployment overrides it via the MAIN_DOMAIN env var — e.g. a free pilot
# deployment on a wildcard-DNS domain that isn't 'bizal.al'.
...
# '/api/tenants/trial-expired' removed — this API path does not exist
...
'/health', # healthcheck must bypass tenant resolution
)
# Pre-compute the stripped set once at module load instead of
...
class TenantMiddleware:
    ...
    def _enforce_trial(self, request):
        ...
        # TTL reduced from 300s to 60s. The webhook handlers for
        ...
        if not cache.get(cache_key):
            # Wrap the is_active=True check and UPDATE in an atomic
            ...
            # Always set the trial_expired cache key on a cache miss,
    ...
    def _resolve_tenant(self, request):
        ...
        # Reference the module-level _BYPASS_SET (computed once at
        # import time) instead of rebuilding the set on every request.
        # Use exact path match, not endswith(), to prevent path-
        # traversal bypass (e.g. /malicious/api/payments/webhook would
        # incorrectly pass the old endswith() check).
        # Bypass paths must be checked BEFORE any port/subdomain

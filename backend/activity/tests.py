...
class ActivityLogTests(TestCase):
    ...
    def test_filter_by_target_type(self):
        log_activity(self.tenant, self.owner, 'booking.confirmed', 'Confirmed booking #1',
                     target_type='booking', target_id='b1')
        log_activity(self.tenant, self.owner, 'staff.added', 'Added staff member',
                     target_type='staff', target_id='s1')
        self.client.force_authenticate(user=self.owner)
        resp = self.client.get('/api/activity/?target_type=booking')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data['results']), 1)
        self.assertEqual(resp.data['results'][0]['description'], 'Confirmed booking #1')
...
class ActivityLogStrTests(TestCase):
    def setUp(self):
        self.tenant = Tenant.objects.create(
            name='Str Co', slug='strco-gap', business_type='restaurant', plan='pro', is_active=True,
        )

    def test_str_uses_actor_name_when_present(self):
        entry = ActivityLog.objects.create(
            tenant=self.tenant, actor_name='Ana', verb='booking.confirmed',
            description='Confirmed booking #1',
        )
        self.assertEqual(str(entry), 'Ana: Confirmed booking #1')

    def test_str_falls_back_to_system_when_no_actor_name(self):
        entry = ActivityLog.objects.create(
            tenant=self.tenant, actor_name='', verb='booking.confirmed',
            description='Confirmed booking #2',
        )
        self.assertEqual(str(entry), 'System: Confirmed booking #2')


class LogActivityExceptionGapsTests(TestCase):
    def test_write_failure_is_silenced_and_returns_none(self):
        tenant = Tenant.objects.create(
            name='Act Co', slug='actco-gap', business_type='restaurant',
            plan='pro', is_active=True,
        )
        with patch('activity.models.ActivityLog.objects.create', side_effect=Exception('db down')):
            result = log_activity(tenant, None, 'booking.confirmed', 'Confirmed booking #1')

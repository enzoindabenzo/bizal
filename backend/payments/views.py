...
def _plan_price_map():
    # Built lazily so values are read at call time, not at import
...
def subscribe(request):
    ...
    # Validate that the Price ID for this plan is actually configured.
    ...
    # Mirror the trial_expired guard already present in
...
def stripe_webhook(request):
    ...
    try:
        ...
        # Delete the idempotency key so Stripe's retry gets a
        ...
        # Log the full exception server-side; return a generic
    ...
    if event_type == 'checkout.session.completed':
        ...
        if slug and plan:
            try:
                ...
                # Capture whether we were on trial BEFORE overwriting
                ...
                # Wrap tenant.save() and Payment.objects.create()
                ...
                with transaction.atomic():
                    ...
                    # Use update_or_create instead of create so that
        ...
        if sub_id:
            try:
                ...
                # Stripe_subscription_id must be cleared INSIDE the atomic
                ...
                with transaction.atomic():
                    ...
                    tenant.save()
                    # Use update_or_create with tenant= + stripe_payment_intent=
        ...
        price_to_plan = {v: k for k, v in _plan_price_map().items() if v} # skip empty-string keys (unconfigured env)
        ...
        if sub_id:
            try:
                ...
                # When payment_intent is empty (card declined before
                ...
                with transaction.atomic():
                    # Switch from get_or_create to update_or_create so
...
class WebhookEventListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # NOTE: WebhookEvent.objects.all() is intentional — this is a cross-tenant
        ...
        if type_f:
            # Event_type values are machine-generated Stripe strings
        ...
        # Removed [:200] slice. The slice was evaluated before DRF's

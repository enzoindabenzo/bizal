...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = (
            'id', 'user', 'user_name', 'booking', 'invoice', 'amount', 'currency',
...
class ManualPaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = (
            'id', 'booking', 'invoice', 'amount', 'currency',
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Scope booking/invoice FKs to the current tenant, matching the same
        # IDOR-prevention pattern InvoiceSerializer uses for 'customer' —
        # otherwise a staff member could record a payment against another
        # tenant's booking/invoice by guessing its UUID.

...
class RentalSearchFilterTests(TestCase):
    """Covers RentalItemListView.get_queryset()'s ?search= Q-filter (lines 54-55)."""

    def setUp(self):
        self.tenant = make_tenant('searchrentals')
        self.client = APIClient()
        self.client.defaults['HTTP_HOST'] = 'searchrentals.bizal.al'
        make_item(self.tenant, 'Convertible Mustang', 'car', 8000, 'Vlorë')
        make_item(self.tenant, 'City Scooter', 'scooter', 1500, 'Tiranë')

    def test_search_matches_name(self):
        resp = self.client.get('/api/rentals/', {'search': 'Mustang'})
        names = [r['name'] for r in resp.data['results']]
        self.assertIn('Convertible Mustang', names)
        self.assertNotIn('City Scooter', names)

    def test_search_matches_city(self):
        resp = self.client.get('/api/rentals/', {'search': 'Tiranë'})
        names = [r['name'] for r in resp.data['results']]
        self.assertIn('City Scooter', names)
        self.assertNotIn('Convertible Mustang', names)

    def test_search_matches_description(self):
        item = make_item(self.tenant, 'Van', 'car', 5000, 'Durrës')
        item.description = 'great for weekend getaways'
        item.save(update_fields=['description'])
        resp = self.client.get('/api/rentals/', {'search': 'getaways'})
        names = [r['name'] for r in resp.data['results']]
        self.assertIn('Van', names)


class CheckAvailabilityGapTests(TestCase):
    """Covers check_availability's invalid-date (110-111) and 404 (114-115) branches."""

    def setUp(self):
        self.tenant = make_tenant('availrentals')
        self.item = make_item(self.tenant)
        self.client = APIClient()
        self.client.defaults['HTTP_HOST'] = 'availrentals.bizal.al'

    def test_invalid_date_format_returns_400(self):
        resp = self.client.get(
            f'/api/rentals/{self.item.id}/availability/',
            {'start_date': 'not-a-date', 'end_date': '2026-08-10'},
        )
    ...
    def test_nonexistent_item_returns_404(self):
        import uuid
        resp = self.client.get(
            f'/api/rentals/{uuid.uuid4()}/availability/',
            {'start_date': '2026-08-01', 'end_date': '2026-08-10'},
        )
        self.assertEqual(resp.status_code, 404)

    def test_valid_dates_and_existing_item_returns_availability(self):
        resp = self.client.get(
            f'/api/rentals/{self.item.id}/availability/',
            {'start_date': '2026-08-01', 'end_date': '2026-08-10'},
        )
        ...
        self.assertEqual(resp.data['item'], self.item.name)


class BookedRangesGapTests(TestCase):
    """Covers booked_ranges (lines 132-144): 404 branch and the successful
    query/response-shaping branch."""

    def setUp(self):
        self.tenant = make_tenant('bookedrentals')
        self.item = make_item(self.tenant)
        self.client = APIClient()
        self.client.defaults['HTTP_HOST'] = 'bookedrentals.bizal.al'

    def test_nonexistent_item_returns_404(self):
        import uuid
        resp = self.client.get(f'/api/rentals/{uuid.uuid4()}/booked-ranges/')
        self.assertEqual(resp.status_code, 404)

    def test_returns_ranges_for_pending_confirmed_active_bookings_only(self):
        from bookings.models import Booking
        Booking.objects.create(
            tenant=self.tenant, resource_id=str(self.item.pk), resource_type='rental_item',
            booking_type='rental', status='confirmed',
            start_date=datetime.date(2026, 8, 1), end_date=datetime.date(2026, 8, 5),
            guest_name='Guest', guest_phone='0691234567',
        )
        Booking.objects.create(
            tenant=self.tenant, resource_id=str(self.item.pk), resource_type='rental_item',
            booking_type='rental', status='cancelled',
            start_date=datetime.date(2026, 9, 1), end_date=datetime.date(2026, 9, 5),
            guest_name='Guest2', guest_phone='0691234568',
        )
        resp = self.client.get(f'/api/rentals/{self.item.id}/booked-ranges/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data['item'], self.item.name)
        self.assertEqual(len(resp.data['ranges']), 1)
        self.assertEqual(str(resp.data['ranges'][0]['start_date']), '2026-08-01')

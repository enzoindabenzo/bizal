...
class RoomBookingListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        # BUG FIX: explicit order_by() so this paginated endpoint's ordering
        # doesn't silently depend on RoomBooking.Meta.ordering staying set.
        # Pagination without a deterministic order can return duplicate or
        # missing rows across pages under concurrent writes.
        return RoomBooking.objects.filter(
            room__tenant=self.request.tenant,
        ).select_related('room__room_type', 'booking').order_by('-id')
...
def room_type_booked_ranges(request, pk):
    ...
    if events:
        ...
        if range_start is not None:  # pragma: no cover - defensive only; every
            # booking carries an end_date, so the sweep always closes out an
            # open range before the loop ends. Kept as a guard against a
            # future data model change (e.g. an open-ended booking) rather
            # than a reachable branch today.

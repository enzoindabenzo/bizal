...
class RoomBooking(models.Model):
    ...
    class Meta:
        ...
        # BUG FIX: without an explicit ordering, Postgres returns rows in
        # whatever order the query planner picks (index scan vs seq scan vs
        # bitmap heap scan), which is not stable across requests once the
        # table sees concurrent writes/autovacuum. RoomBookingListCreateView
        # paginates this queryset, so an unordered default meant an owner
        # paging through results could see duplicate rows on one page and
        # missing rows on another — silently, with no error. Every other
        # model in this codebase defines Meta.ordering; this one was the
        # sole exception. -id is a stable, monotonically increasing tiebreak
        # (no created_at field exists on this thin join table by design).
        ordering = ['-id']

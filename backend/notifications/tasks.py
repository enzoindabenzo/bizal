...
def send_notification_email(self, user_id, subject, html_body, from_email=None):
    ...
    try:
        ...
        # Catch MaxRetriesExceededError so the final permanent
...
"""
        ...
        # This task retries up to 2 times (see @shared_task above).
        ...
        # Log permanent failure when retries are exhausted.
        try:
            raise self.retry(exc=exc)
        except MaxRetriesExceededError:
            logger.error(
...
def send_appointment_reminders():
    ...
    # Datetime.date.today() returns system-local date (UTC in Docker).
    ...
    reminder_attempts__lt=3, # skip after 3 failed attempts
    ).select_related('tenant', 'user', 'service')

    sent = 0
    for appt in appointments:
        # Skip reminders for deactivated/anonymised users. A deactivated
        ...
        try:
            # Fail_silently=False so SMTP errors raise and are caught
            ...
            # Atomic conditional update — only mark as sent if
            ...
            if updated:
                sent += 1
            # NOTE: there is a small split-brain window between send_mail()
...
def send_sms_stub(phone_number, message):
    ...
    Previously this task logged a line and returned SUCCESS,
...
def notify_owner_async(self, tenant_id, notification_type, title, body, metadata=None, idempotency_key=''):
    ...
    this task previously had no retry handling at all (a
...
def send_booking_reminders():
    ...
    # Same timezone fix as send_appointment_reminders above.
    tomorrow = _tz.localdate() + datetime.timedelta(days=1)
    ...
    for booking in bookings:
        # Skip reminders for deactivated/anonymised users (mirrors
        ...
        try:
            ...
            # Atomic conditional update — see matching comment
            ...
            if updated:
                sent += 1
            # NOTE: same split-brain window as send_appointment_reminders —

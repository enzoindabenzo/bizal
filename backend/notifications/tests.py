...
class NotificationUtilsTest(TestCase):
    ...
    def test_notify_owner_with_same_key_is_idempotent(self):
        """
        Calling notify_owner() twice with the same
...
def make_tenant__gaps(slug):
    return Tenant.objects.create(name=slug.title(), slug=slug, plan='enterprise', is_active=True, business_type='retail')
...
class NotificationFilterTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.tenant = make_tenant__gaps('filterbiz')
        self.user = make_user__gaps('u@filterbiz.com', self.tenant)
        self.client.defaults['HTTP_HOST'] = 'filterbiz.bizal.al'
        Notification.objects.create(
            tenant=self.tenant, user=self.user,
            notification_type='info', title='N1', body='B1', is_read=False,
        )
        Notification.objects.create(
            tenant=self.tenant, user=self.user,
            notification_type='booking_confirmed', title='N2', body='B2', is_read=True,
        )
        self.client.force_authenticate(user=self.user)

    def test_filter_by_notification_type(self):
        resp = self.client.get('/api/notifications/?notification_type=info')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        results = resp.data['results'] if isinstance(resp.data, dict) else resp.data
        titles = [n['title'] for n in results]
        self.assertIn('N1', titles)
        self.assertNotIn('N2', titles)

    def test_filter_unread_true(self):
...
def make_tenant__tasks_extra(slug, plan='pro', is_active=True):
    return Tenant.objects.create(
        name=slug.title(), slug=slug, plan=plan, is_active=is_active, business_type='restaurant',
    )


def make_user__tasks_extra(email, tenant, role='customer'):
    return User.objects.create_user(email=email, password='pass1234', tenant=tenant, role=role)


class SendNotificationEmailTests(TestCase):
    def setUp(self):
        self.tenant = make_tenant__tasks_extra('notiftask')
        self.user = make_user__tasks_extra('notify@notiftask.com', self.tenant)

    @patch('notifications.tasks.send_mail')
    def test_sends_email_with_plain_text_fallback(self, mock_send_mail):
        from notifications.tasks import send_notification_email
        send_notification_email(str(self.user.pk), 'Hello', '<p>Hi <b>there</b></p>')
        mock_send_mail.assert_called_once()
        kwargs = mock_send_mail.call_args[1]
        self.assertEqual(kwargs['subject'], 'Hello')
        self.assertIn('Hi there', kwargs['message'])
        self.assertEqual(kwargs['recipient_list'], [self.user.email])

    @patch('notifications.tasks.send_mail')
    def test_skips_user_with_no_email(self, mock_send_mail):
        from notifications.tasks import send_notification_email
        self.user.email = ''
        self.user.save(update_fields=['email'])
        send_notification_email(str(self.user.pk), 'Hello', '<p>Hi</p>')
        mock_send_mail.assert_not_called()

    @patch('notifications.tasks.send_mail')
    def test_nonexistent_user_retries_and_raises_after_exhaustion(self, mock_send_mail):
        from notifications.tasks import send_notification_email
        with self.assertRaises(Exception):
            send_notification_email.apply(args=('00000000-0000-0000-0000-000000000000', 'Hi', '<p>x</p>')).get()

    @patch('notifications.tasks.send_mail', side_effect=Exception('smtp down'))
    def test_smtp_failure_logs_permanent_failure_after_retries(self, mock_send_mail):
        from notifications.tasks import send_notification_email
        with self.assertRaises(Exception):
            send_notification_email.apply(args=(str(self.user.pk), 'Hi', '<p>x</p>')).get()


class SendBookingConfirmationEmailTests(TestCase):
    def setUp(self):
        self.tenant = make_tenant__tasks_extra('bookconfirm')

    def _make_booking(self, **kwargs):
        from bookings.models import Booking
        defaults = dict(
            tenant=self.tenant, booking_type='table', status='confirmed',
            guest_name='Ana', guest_email='ana@example.com', start_date=datetime.date.today(),
        )
        defaults.update(kwargs)
        return Booking.objects.create(**defaults)

    @patch('notifications.utils.notify_owner')
    @patch('notifications.tasks.send_mail')
    def test_sends_guest_email_and_notifies_owner(self, mock_send_mail, mock_notify_owner):
        from notifications.tasks import send_booking_confirmation_email
        booking = self._make_booking()
        send_booking_confirmation_email(str(booking.pk))
        mock_send_mail.assert_called_once()
        mock_notify_owner.assert_called_once()

    @patch('notifications.utils.notify_owner')
    @patch('notifications.tasks.send_mail')
    def test_falls_back_to_user_email_when_no_guest_email(self, mock_send_mail, mock_notify_owner):
        from notifications.tasks import send_booking_confirmation_email
        user = make_user__tasks_extra('userbook@bookconfirm.com', self.tenant)
        booking = self._make_booking(guest_email='', user=user)
        send_booking_confirmation_email(str(booking.pk))
        kwargs = mock_send_mail.call_args[1]
    ...
    @patch('notifications.utils.notify_owner')
    @patch('notifications.tasks.send_mail')
    def test_no_email_at_all_skips_send_mail_but_still_notifies_owner(self, mock_send_mail, mock_notify_owner):
        from notifications.tasks import send_booking_confirmation_email
        booking = self._make_booking(guest_email='')
        send_booking_confirmation_email(str(booking.pk))
        mock_send_mail.assert_not_called()
        mock_notify_owner.assert_called_once()

    def test_nonexistent_booking_retries_and_raises(self):
        from notifications.tasks import send_booking_confirmation_email
        with self.assertRaises(Exception):
            send_booking_confirmation_email.apply(args=('00000000-0000-0000-0000-000000000000',)).get()


class SendSmsStubTests(TestCase):
    def test_raises_not_implemented(self):
        from notifications.tasks import send_sms_stub
        with self.assertRaises(NotImplementedError):
            send_sms_stub('+355691234567', 'Test message')


class NotifyOwnerAsyncTests(TestCase):
    def setUp(self):
        self.tenant = make_tenant__tasks_extra('notifyasync')
        self.owner = make_user__tasks_extra('owner@notifyasync.com', self.tenant, role='owner')

    @patch('notifications.utils.notify_owner')
    def test_notifies_owner_for_existing_tenant(self, mock_notify_owner):
        from notifications.tasks import notify_owner_async
        notify_owner_async(str(self.tenant.pk), 'booking_confirmed', 'Title', 'Body')
        mock_notify_owner.assert_called_once()

    def test_returns_silently_for_nonexistent_tenant(self):
        from notifications.tasks import notify_owner_async
        import uuid
        result = notify_owner_async(str(uuid.uuid4()), 'booking_confirmed', 'Title', 'Body')
        self.assertIsNone(result)

    @patch('notifications.utils.notify_owner', side_effect=Exception('db error'))
    def test_retries_and_raises_after_exhaustion_on_error(self, mock_notify_owner):
        from notifications.tasks import notify_owner_async
        with self.assertRaises(Exception):
            notify_owner_async.apply(args=(str(self.tenant.pk), 'booking_confirmed', 'T', 'B')).get()


class SendBookingRemindersTests(TestCase):
    def setUp(self):
        self.active_tenant = make_tenant__tasks_extra('activebook')
        self.inactive_tenant = make_tenant__tasks_extra('inactivebook', is_active=False)
        self.tomorrow = datetime.date.today() + datetime.timedelta(days=1)

    def _make_booking(self, tenant, **kwargs):
        from bookings.models import Booking
        defaults = dict(
            tenant=tenant, booking_type='table', status='confirmed',
            start_date=self.tomorrow, reminder_sent=False,
        )
        defaults.update(kwargs)
        return Booking.objects.create(**defaults)

    @patch('notifications.tasks.send_mail')
    def test_sends_reminder_for_guest_email(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        self._make_booking(self.active_tenant, guest_name='Gu', guest_email='gu@example.com')
        result = send_booking_reminders()
        mock_send_mail.assert_called_once()
        self.assertIn('1', result)

    @patch('notifications.tasks.send_mail')
    def test_skips_inactive_tenant_bookings(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        self._make_booking(self.inactive_tenant, guest_name='In', guest_email='in@example.com')
        send_booking_reminders()
        mock_send_mail.assert_not_called()

    @patch('notifications.tasks.send_mail')
    def test_skips_booking_with_no_contact_email(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        self._make_booking(self.active_tenant)
        try:
            send_booking_reminders()
        except Exception as exc:
            self.fail(f'send_booking_reminders raised unexpectedly: {exc}')
        mock_send_mail.assert_not_called()

    @patch('notifications.tasks.send_mail')
    def test_skips_deactivated_user(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        user = make_user__tasks_extra('deactivated@activebook.com', self.active_tenant)
        user.is_active = False
        user.save(update_fields=['is_active'])
        self._make_booking(self.active_tenant, user=user, guest_email='ignored@example.com')
        send_booking_reminders()
        mock_send_mail.assert_not_called()

    @patch('notifications.tasks.send_mail')
    def test_user_email_path_used_over_guest_email(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        user = make_user__tasks_extra('loggedin@activebook.com', self.active_tenant)
        self._make_booking(self.active_tenant, user=user, guest_email='ignored@example.com')
        send_booking_reminders()
        kwargs = mock_send_mail.call_args[1]
        self.assertEqual(kwargs['recipient_list'], [user.email])
    ...
    @patch('notifications.tasks.send_mail')
    def test_does_not_resend_already_sent_reminder(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        self._make_booking(
            self.active_tenant, guest_name='Al', guest_email='al@example.com', reminder_sent=True,
        )
        send_booking_reminders()
        mock_send_mail.assert_not_called()

    @patch('notifications.tasks.send_mail', side_effect=Exception('smtp down'))
    def test_smtp_failure_increments_attempts_and_does_not_raise(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        from bookings.models import Booking
        booking = self._make_booking(self.active_tenant, guest_name='Er', guest_email='er@example.com')
        try:
            send_booking_reminders()
        except Exception as exc:
            self.fail(f'send_booking_reminders raised unexpectedly: {exc}')
        booking.refresh_from_db()
        self.assertEqual(booking.reminder_attempts, 1)
        self.assertFalse(booking.reminder_sent)

    @patch('notifications.tasks.send_mail')
    def test_skips_after_3_failed_attempts(self, mock_send_mail):
        from notifications.tasks import send_booking_reminders
        self._make_booking(
            self.active_tenant, guest_name='Mx', guest_email='mx@example.com', reminder_attempts=3,
        )
        send_booking_reminders()
        mock_send_mail.assert_not_called()


def make_tenant__tasks_gaps2(slug, plan='pro', is_active=True):
    return Tenant.objects.create(
        name=slug.title(), slug=slug, plan=plan, is_active=is_active, business_type='clinic',
    )


class SendNotificationEmailMaxRetriesTests(TestCase):
    @patch('notifications.tasks.send_mail', side_effect=Exception('smtp down'))
    @patch('notifications.tasks.send_notification_email.retry', side_effect=MaxRetriesExceededError())
    def test_logs_permanent_failure_when_retries_exhausted(self, mock_retry, mock_send_mail):
        from notifications.tasks import send_notification_email
        tenant = make_tenant__tasks_gaps2('maxretry1')
        user = User.objects.create_user(email='mr1@maxretry1.com', password='pass1234', tenant=tenant)
        with self.assertRaises(MaxRetriesExceededError):
            send_notification_email.apply(args=(str(user.pk), 'Hi', '<p>x</p>')).get()


class SendBookingConfirmationEmailMaxRetriesTests(TestCase):
    @patch('notifications.tasks.send_booking_confirmation_email.retry', side_effect=MaxRetriesExceededError())
    def test_logs_permanent_failure_when_retries_exhausted(self, mock_retry):
        from notifications.tasks import send_booking_confirmation_email
        # Nonexistent booking id -> Booking.DoesNotExist -> except Exception -> retry
        with self.assertRaises(MaxRetriesExceededError):
            send_booking_confirmation_email.apply(args=('00000000-0000-0000-0000-000000000000',)).get()


class NotifyOwnerAsyncMaxRetriesTests(TestCase):
    @patch('notifications.utils.notify_owner', side_effect=Exception('db error'))
    @patch('notifications.tasks.notify_owner_async.retry', side_effect=MaxRetriesExceededError())
    def test_logs_permanent_failure_when_retries_exhausted(self, mock_retry, mock_notify_owner):
        from notifications.tasks import notify_owner_async
        tenant = make_tenant__tasks_gaps2('maxretry2')
        with self.assertRaises(MaxRetriesExceededError):
            notify_owner_async.apply(args=(str(tenant.pk), 'booking_confirmed', 'T', 'B')).get()


class SendAppointmentRemindersGapsTests(TestCase):
    def setUp(self):
        self.tenant = make_tenant__tasks_gaps2('apptremgap')
        from appointments.models import ServiceProvider, Service
        self.provider = ServiceProvider.objects.create(tenant=self.tenant, name='Dr. G', is_active=True)
        self.service = Service.objects.create(
            tenant=self.tenant, name='Check', duration_minutes=30, price=1000, is_active=True,
        )
        self.tomorrow = datetime.date.today() + datetime.timedelta(days=1)
    ...
    def test_skips_deactivated_user(self, mock_send_mail):
        ...
        user = User.objects.create_user(
            email='deact@apptremgap.com', password='pass1234', tenant=self.tenant, role='customer',
        )
        user.is_active = False
        user.save(update_fields=['is_active'])
        Appointment.objects.create(
            tenant=self.tenant, service=self.service, provider=self.provider, user=user,
            date=self.tomorrow, start_time=datetime.time(9, 0), end_time=datetime.time(9, 30),
            status='confirmed', guest_email='ignored@example.com',
        )
        from notifications.tasks import send_appointment_reminders
        send_appointment_reminders()
        mock_send_mail.assert_not_called()
    ...
    @patch('notifications.tasks.send_mail', side_effect=Exception('smtp down'))
    def test_send_failure_increments_attempts_and_does_not_raise(self, mock_send_mail):
        from appointments.models import Appointment
        appt = Appointment.objects.create(
            tenant=self.tenant, service=self.service, provider=self.provider,
            date=self.tomorrow, start_time=datetime.time(9, 0), end_time=datetime.time(9, 30),
            status='confirmed', guest_name='Er', guest_email='er@example.com',
        )
        from notifications.tasks import send_appointment_reminders
        try:
            send_appointment_reminders()
        ...
        self.assertEqual(appt.reminder_attempts, 1)

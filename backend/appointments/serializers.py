...
class ServiceSerializer(serializers.ModelSerializer):
    class Meta:
        model = Service
        # 'providers' exposes the M2M link so the tenant admin panel can
        # actually assign which ServiceProvider(s) can perform a service.
        # This was previously omitted, so a tenant had no way to connect
        # services to providers through the API even though the model and
        # the public booking flow both depend on that link.
        fields = ('id', 'name', 'description', 'duration_minutes', 'price', 'is_active', 'providers')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Scope the providers choices to the current tenant, same pattern as
        # ServiceProviderSerializer.staff_member — otherwise an owner could
        # (accidentally or otherwise) link a service to another tenant's
        # provider via a guessed UUID.
        request = self.context.get('request')
        if request and hasattr(request, 'tenant') and request.tenant:
            self.fields['providers'].queryset = ServiceProvider.objects.filter(
                tenant=request.tenant
            )

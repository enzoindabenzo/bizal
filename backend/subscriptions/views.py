...
from tenants.permissions import HasTenantRole, HasPlanAtLeast, TenantDomainOnly
...
class SubscriptionListCreateView(generics.ListCreateAPIView):
    ...
    # HasPlanAtLeast added: tenant_admin.html only shows the "Abonimet" nav
    # item for pro/enterprise plans, but the backend had no matching plan
    # check — only a role check — so a Starter-plan tenant's accountant
    # could call this endpoint directly and use the feature for free.
    permission_classes = [HasTenantRole('accountant'), HasPlanAtLeast('pro', 'enterprise')]
...
class MySubscriptionsView(generics.ListAPIView):
    ...
    Added TenantDomainOnly to permission_classes. Previously, a

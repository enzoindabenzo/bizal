...
def make_tenant(slug, plan='pro', business_type='gym'):
    return Tenant.objects.create(
        name=slug.title(), slug=slug, plan=plan, business_type=business_type, is_active=True,
    )
...
class SubscriptionDetailUpdateGapsTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.tenant = make_tenant('gym-update-gap')
        self.other_tenant = make_tenant('gym-update-gap-other')
        self.owner = make_user('owner@gym-update-gap.com', self.tenant)
        self.customer = make_user('cust@gym-update-gap.com', self.tenant, role='customer')
        self.other_customer = make_user('cust@gym-update-gap-other.com', self.other_tenant, role='customer')
        self.client.defaults['HTTP_HOST'] = 'gym-update-gap.bizal.al'
        self.client.force_authenticate(user=self.owner)
        self.sub = CustomerSubscription.objects.create(
            tenant=self.tenant, customer=self.customer,
            name='Monthly Pass', price=3000, frequency='monthly', status='active',
        )

    def test_owner_can_update_own_subscription(self):
        resp = self.client.patch(f'/api/subscriptions/{self.sub.pk}/', {'status': 'cancelled'})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.sub.refresh_from_db()
        self.assertEqual(self.sub.status, 'cancelled')

    def test_reassigning_to_cross_tenant_customer_is_rejected(self):
        resp = self.client.patch(f'/api/subscriptions/{self.sub.pk}/', {
            'customer': str(self.other_customer.pk),
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.sub.refresh_from_db()
        self.assertEqual(self.sub.customer_id, self.customer.pk)


class SubscriptionPlanGateTests(TestCase):
    """
    Regression test: the tenant admin nav only shows "Abonimet" (subscriptions)
    for pro/enterprise plans, but the owner-facing endpoints previously had no
    matching plan check on the backend — only a role check — so a Starter-plan
    tenant's owner/accountant could call the API directly and use the feature
    for free. HasPlanAtLeast('pro', 'enterprise') closes that gap.
    """
    def setUp(self):
        self.client = APIClient()
        self.starter_tenant = Tenant.objects.create(
            name='Starter Gym', slug='starter-gym', business_type='gym', plan='starter',
            is_active=True,
        )
        self.owner = User.objects.create_user(
            email='owner@startergym.com', password='pass1234', tenant=self.starter_tenant, role='owner',
        )
        self.client.defaults['HTTP_HOST'] = 'starter-gym.bizal.al'
        self.client.force_authenticate(self.owner)

    def test_starter_plan_owner_cannot_list_subscriptions(self):
        resp = self.client.get('/api/subscriptions/')
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_starter_plan_owner_cannot_create_subscription(self):
        resp = self.client.post('/api/subscriptions/', {
            'customer': str(self.owner.pk), 'name': 'Monthly Pass', 'price': 2000,
        })
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

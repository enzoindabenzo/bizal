...
# Transition guard mirrors the same pattern used for booking status transitions.
...
class OrderListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Short-circuit to none() on the main domain (tenant=None).
        ...
        # Use get_effective_role() — respects StaffMember.is_active.
    ...
    def perform_create(self, serializer):
        ...
        # Reject unauthenticated POST from the main domain where
        ...
        # REVERTED: The original gate blocked ALL order creation for
...
def admin_update_order(request, pk):
    ...
    update_fields = []
    new_status = None # initialize before conditional so line 95 never raises NameError
    ...
    if not update_fields:
        # Nothing was supplied — return current state without a pointless DB write
    ...
    with transaction.atomic():
        ...
        if new_status == 'delivered':  # check transition, not current status
            # Mirror the bookings loyalty log pattern.
        ...
        if new_status == 'cancelled':
            ...
            if product_items:
                ...
                for pi in product_items:
                    ...
                    Product.objects.filter(pk=pi.product_id, tenant=request.tenant).update(stock=F('stock') + pi.quantity)
                try:

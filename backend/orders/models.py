...
class OrderItem(UUIDModel):
    ...
    class Meta:
        # LOW-severity fix: order.items.all() (nested in OrderSerializer)
        # had no default ordering, so line items could visibly reshuffle
        # between page loads. OrderItem has no created_at field (only a
        # random UUID pk), so 'id' isn't insertion order, but it is at
        # least stable across requests, which is the actual bug being
        # fixed here — not accurate chronological ordering.
        ordering = ['id']
        constraints = [

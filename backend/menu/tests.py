...
def make_tenant__gaps2(slug='menugapbiz'):
    return Tenant.objects.create(name=slug.title(), slug=slug, plan='pro', is_active=True, business_type='restaurant')


def make_user__gaps2(email, tenant, role='owner'):
    return User.objects.create_user(email=email, password='pass1234', tenant=tenant, role=role)


class MenuProtectedErrorTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.tenant = make_tenant__gaps2('menuprotectbiz')
        self.owner = make_user__gaps2('owner@menuprotectbiz.com', self.tenant)
        self.category = MenuCategory.objects.create(tenant=self.tenant, name='Mains')
        self.item = MenuItem.objects.create(tenant=self.tenant, category=self.category, name='Burger', price=10)
        self.order = Order.objects.create(tenant=self.tenant, order_type='dine_in', guest_name='G')
        OrderItem.objects.create(order=self.order, menu_item=self.item, quantity=1, unit_price=10)
        self.client.defaults['HTTP_HOST'] = 'menuprotectbiz.bizal.al'
        self.client.force_authenticate(user=self.owner)

    def test_item_delete_blocked_by_existing_order_returns_409(self):
        resp = self.client.delete(f'/api/menu/items/{self.item.pk}/')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue(MenuItem.objects.filter(pk=self.item.pk).exists())

    def test_category_delete_blocked_by_ordered_item_returns_409(self):
        resp = self.client.delete(f'/api/menu/categories/{self.category.pk}/')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

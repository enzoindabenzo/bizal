...
def purge_old_events(days=90, batch_size=25000):
    ...
    Previously deleted all matching rows in a single transaction,
    ...
    The previous global ordering by (created_at, id) meant that
    ...
    if tenant_ids:
        ...
        for tenant_id in tenant_ids:
            # Break the outer loop too when the batch budget is
            ...
            if batches_used >= MAX_BATCHES:  # pragma: no cover - safety valve
                # only reachable after MAX_BATCHES (10,000) delete batches in a
                # single task run; not practically exercisable in a unit test.
                break
            for _ in range(per_tenant_batches):
                if batches_used >= MAX_BATCHES:  # pragma: no cover - same
                    # 10,000-batch safety valve as above, checked per-tenant.
                    break
                ids = list(

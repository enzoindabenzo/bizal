...
def analytics_dashboard(request):
    ...
    # Validate date format before hitting the ORM to return 400 not 500.
...
class TrackEventView(APIView):
    ...
    def post(self, request):
        ...
        # AnalyticsEvent.event_type has `choices=EVENT_TYPES`, but
...
def export_bookings_csv(request):
    ...
    previously gated only by IsTenantOwner, unlike the inline CSV
...
def export_customers_csv(request):
    ...
    gated on both 'csv_export' (matches the other two export

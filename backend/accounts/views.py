...
class RegisterView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            # Use the email-verification-specific generator, not the
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    @method_decorator(_ratelimit_decorator('5/m'))
    def post(self, request, *args, **kwargs):
        # Validate inline on a single serializer instance so that
...
class ChangePasswordView(APIView):
    ...
    def post(self, request):
        ...
        # Blacklist all outstanding refresh tokens so that active
        ...
        try:
            ...
            for t in OutstandingToken.objects.filter(user=user):
                try:
                    ...
                    # Individual token may already be expired/blacklisted — log but continue
                    logger.exception('ChangePasswordView: failed to blacklist token %s for user %s', t.pk, user.pk)
        except Exception:
            # Blacklisting must not silently fail — log so operators can detect revocation gaps
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Replace the fixed sentinel address
            ...
            # Use a realistic bcrypt/argon2-length password hash for
        ...
        # Always route reset links to the main domain. The previous
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request):
        ...
        # Blacklist all outstanding refresh tokens so that an
...
class MeBookingsView(APIView):
    ...
    def get(self, request):
        # Add select_related('tenant') so BookingSerializer can
...
class MeOrdersView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Re-raise database schema errors (unapplied
...
class MeAppointmentsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Re-raise database schema errors so Django's
...
class MeDeleteView(APIView):
    ...
    def delete(self, request):
        ...
        # Delete the avatar file from storage before anonymising so
        ...
        user.full_name = ''
        user.phone = ''
        # Clear city and business_name — both are user-entered PII
        ...
        user.city = ''
        user.business_name = ''
        # Reset is_email_verified so anonymised accounts don't
        ...
        try:
            ...
            # Log revocation failures; account is anonymised but tokens may still mint access tokens
...
class MeNotificationPrefsView(APIView):
    ...
    def patch(self, request):
        ...
        # Prevent storage abuse — an authenticated user could send
...
class EmailVerificationSendView(APIView):
    ...
    # Add per-user rate limit to prevent SMTP quota exhaustion if a
    ...
    def post(self, request):
        ...
        uid   = urlsafe_base64_encode(force_bytes(user.pk))
        # Dedicated generator — see accounts/tokens.py.
        from .tokens import email_verification_token_generator
        token = email_verification_token_generator.make_token(user)
        base  = django_settings.FRONTEND_BASE_URL
        link  = f"{base}/verify-email/{uid}/{token}/"
...
class EmailVerificationConfirmView(APIView):
    ...
    def get(self, request, uid, token):
        ...
        # Reject verification attempts for deactivated/anonymised
        ...
        # Check against the email-verification-specific generator,

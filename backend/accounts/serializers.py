...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        # Normalize the email to lowercase at the serializer level so
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        token['is_staff'] = user.is_staff # required by frontend role-gating (see index.html/tenant_admin.html ROLE checks)
        return token

    def validate(self, attrs):
        # Lowercase the submitted username (email) before the parent

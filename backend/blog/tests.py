...
class BlogPublicReadTest(TestCase):
    ...
    def test_public_list_does_not_n_plus_1_on_author_and_tags(self):
        """
        Regression test: BlogPostListSerializer reads obj.author.display_name
        per post and nests `tags = BlogTagSerializer(many=True)` per post.
        Without select_related('author') + prefetch_related('tags') on the
        list view's queryset, both scale with the number of posts returned.
        This is the public/anonymous storefront blog endpoint, so it's the
        highest-traffic place this bug could bite. Assert query count stays
        flat as post count (and tags-per-post) grows.
        """
        from django.test.utils import CaptureQueriesContext
        from django.db import connection

        author = make_user('writer@blogbiz.com', self.tenant)
        tag_a = BlogTag.objects.create(tenant=self.tenant, name='News', slug='news')
        tag_b = BlogTag.objects.create(tenant=self.tenant, name='Tips', slug='tips')

        def make_posts(n):
            for i in range(n):
                post = make_post(self.tenant, f'Post {i}', author=author)
                post.tags.set([tag_a, tag_b])
        ...
        BlogPost.objects.filter(tenant=self.tenant, title__startswith='Post ').delete()
        make_posts(15)
        with CaptureQueriesContext(connection) as ctx:
            resp = self.client.get('/api/blog/')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        ...
        self.assertLessEqual(
            ...
            f"Query count grew with post count ({small_count} -> {large_count}); "
            "looks like an N+1 on author/tags.",
        )
...
class BlogPostCoverUrlGapsTests(TestCase):
    def setUp(self):
        self.tenant = Tenant.objects.create(
            name='Blog Co', slug='blogco-gap', business_type='restaurant', plan='pro', is_active=True,
        )

    def test_cover_url_built_with_request_in_context(self):
        post = BlogPost.objects.create(
            tenant=self.tenant, title='Hello', slug='hello', body='Body text',
            cover=SimpleUploadedFile('photo.jpg', b'fake-bytes', content_type='image/jpeg'),
        )

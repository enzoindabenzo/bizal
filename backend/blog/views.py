...
class BlogPostListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # N+1 FIX: BlogPostListSerializer reads obj.author.display_name (one
        # query per post without select_related) and nests
        # `tags = BlogTagSerializer(many=True)` (one query per post without
        # prefetch_related). This is a public, unauthenticated storefront
        # endpoint — the highest-traffic kind of view in this codebase — so
        # an unbounded page of posts was turning into roughly 2x the number
        # of extra queries on every anonymous visit.
        qs = BlogPost.objects.filter(
            tenant=self.request.tenant, status=STATUS_PUBLISHED

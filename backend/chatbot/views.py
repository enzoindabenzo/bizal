...
# Custom key function that falls back to REMOTE_ADDR when X-Real-IP is
...
def _is_trivial(text: str) -> bool:
    ...
    if t in ALL_TRIVIAL:
        return True
    # Removed the blanket `len(t) <= 6 and '?' not in t` rule.
...
def _rotate_call(messages, system_prompt):
    ...
    for lbl, key, fn in providers:
        ...
        try:
            ...
            # V65 Use cache.add() to atomically set the key with TTL
            ...
            try:
                ...
                # V65 cache.add() above guarantees TTL is set before
...
# Tenant.name, tenant.address, tenant.business_hours, staff names,
...
# Named constant so all session-related TTLs are visible together.
...
def _inc_trivial(session_key: str):
    # Use atomic cache.incr() instead of the previous get-then-set
    ...
    #
    # cache.add() guard added before incr(), mirroring the same pattern
    # already used by _rotate_call(). On Redis, cache.incr()
...
@ratelimit(key=_chatbot_ip_key, rate='30/m', method='POST', block=True) # use _chatbot_ip_key (falls back to REMOTE_ADDR) instead of 'header:x-real-ip'
def chat(request):
    try:
    ...
    # Validate incoming session_id with HMAC.
    ...
    if tenant_slug:
        ...
        # Chat() does its own DB lookup bypassing the middleware's
    ...
    if tenant:
        ...
        # Add per-session daily cap for the main-domain chatbot.
        ...
        # Replace non-atomic get+set with cache.incr() so that
        ...
        #
        # cache.add() guard added before incr(), mirroring the same pattern
        # already used by _rotate_call() and _inc_trivial() above. On Redis, cache.incr() on a missing
        ...
        if new_daily_count > 20:
            # Return 429 instead of 200 so monitoring/alerting
    ...
    try:
        ...
        if "all_capped" in err:
            # Return 429 instead of 200, matching the per-session
            # daily-cap path. HTTP 200 on a rate-limit condition
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@ratelimit(key=_chatbot_ip_key, rate='5/m', method='POST', block=True) #
def handoff(request):
    try:
        body = json.loads(request.body)
    except (json.JSONDecodeError, UnicodeDecodeError):
    ...
    # Explicit trial_expired guard (bypasses middleware cache)
    if tenant.trial_expired:
        return JsonResponse({"error": "Trial expired."}, status=402)

    # Require a non-empty, HMAC-verified session_id on every handoff
...
def staff_reply(request):
    ...
    # Verify the user has a staff-level role on this tenant.
    ...
    if not request.user.is_superuser:
        ...
        if effective_role not in ('owner', 'manager', 'staff', 'receptionist', 'accountant'):
            # 'accountant' is a valid staff role used throughout the
    ...
    # Verify the HMAC signature on session_id before writing to
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@ratelimit(key=_chatbot_ip_key, rate='60/m', block=True) #
def poll(request, session_id):
    ...
    Verify the HMAC signature before looking up any cache state.
    ...
    the reply. Uses the module-level _verify_session_token helper ().
    """

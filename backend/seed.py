...
# ── Credential storage ───────────────────────────────────────────────────────
# BUG FIX: this used to fall back to fixed, documented passwords
# (Admin123*, Owner2026!, ...) printed straight to stdout on every run —
# fine for a throwaway local dev box, not fine for a public demo whose
# deploy/console logs anyone with project access can read. Passwords are
# now random per install and are never printed: they're written once to a
# local, git-ignored .env.seed file (KEY=value, same shape as .env) and
# read back from there on every later run, so re-seeding still doesn't
# rotate anyone's login — you just look the value up in the file instead
# of the terminal scrollback. An explicit SEED_*_PASSWORD env var always
# wins over whatever's stored, same as before.
_seed_creds_file = _root / '.env.seed'
_stored_seed_creds = {}
if _seed_creds_file.exists():
    for _line in _seed_creds_file.read_text(encoding='utf-8').splitlines():
        _line = _line.strip()
        if _line and not _line.startswith('#') and '=' in _line:
            _k, _, _v = _line.partition('=')
            _stored_seed_creds[_k.strip()] = _v.strip()

_new_seed_creds = {}  # written out to _seed_creds_file once, at the end

def _seed_password(env_key, label):
    pw = os.environ.get(env_key) or _stored_seed_creds.get(env_key)
    if not pw:
        pw = secrets.token_urlsafe(12)
        print(f"  [seed] {label} password not set in env — generated a new random one (see .env.seed)")
    _new_seed_creds[env_key] = pw
    return pw

ADMIN_PASSWORD    = _seed_password('SEED_ADMIN_PASSWORD',    'admin')
CUSTOMER_PASSWORD = _seed_password('SEED_CUSTOMER_PASSWORD', 'customer')

# Distinct per-owner demo password instead of one password shared across all
# 10 tenant owners — a shared credential meant compromising/leaking any single
# owner login effectively compromised every tenant's admin panel. Each owner
# gets its own random, stored password; SEED_OWNER_PASSWORD still works as a
# blanket override for everyone if you specifically want that, and each
# owner's own SEED_OWNER_PASSWORD_<KEY> env var overrides just that one.
_owner_pw_override = os.environ.get('SEED_OWNER_PASSWORD')

def _owner_password(tenant_label, env_key):
    if _owner_pw_override:
        pw = _owner_pw_override
    else:
        pw = os.environ.get(env_key) or _stored_seed_creds.get(env_key)
        if not pw:
            pw = secrets.token_urlsafe(12)
            print(f"  [seed] owner password for {tenant_label} not set in env — generated a new random one (see .env.seed)")
    _new_seed_creds[env_key] = pw
    return pw

OWNER_PASSWORD_RESTO    = _owner_password('restorant-adriatiku', 'SEED_OWNER_PASSWORD_RESTO')
OWNER_PASSWORD_CARS     = _owner_password('hertz-albania',       'SEED_OWNER_PASSWORD_CARS')
OWNER_PASSWORD_CLINIC   = _owner_password('klinika-shendeti',    'SEED_OWNER_PASSWORD_CLINIC')
OWNER_PASSWORD_BARBER   = _owner_password('barber-kings-tirana', 'SEED_OWNER_PASSWORD_BARBER')
OWNER_PASSWORD_LANGSC   = _owner_password('learning-center',     'SEED_OWNER_PASSWORD_LANGSC')
OWNER_PASSWORD_REALEST  = _owner_password('amos-realestate',     'SEED_OWNER_PASSWORD_REALEST')
OWNER_PASSWORD_TRAVEL   = _owner_password('adriatic-tours',      'SEED_OWNER_PASSWORD_TRAVEL')
OWNER_PASSWORD_CONSTR   = _owner_password('ndertim-shpk',        'SEED_OWNER_PASSWORD_CONSTR')
OWNER_PASSWORD_MARKET   = _owner_password('market-express',      'SEED_OWNER_PASSWORD_MARKET')
OWNER_PASSWORD_HOTEL    = _owner_password('hotel-riviera',       'SEED_OWNER_PASSWORD_HOTEL')
...
# Persist every password used this run (freshly generated ones AND ones
# reused from a prior run / env vars) so the file is always a complete,
# current reference — never partial, never stale.
_seed_creds_lines = [
    "# Auto-generated by backend/seed.py — demo login passwords.",
...
print(f"\nFjalëkalimet u ruajtën (pa u printuar) në: {_seed_creds_file}")
print("   (skedar lokal, nuk komitohet në git — hapeni për t'i parë)")
print("\nEmailet e hyrjes (login emails):")
print("   Superadmin : admin@bizal.al")
print("   Owners (secila tenante ka fjalëkalimin e vet — shih .env.seed):")
print("     owner@adriatiku.al       (restorant-adriatiku)")
print("     owner@hertz.al           (hertz-albania)")
print("     owner@klinikashendeti.al (klinika-shendeti)")
print("     owner@barberkings.al     (barber-kings-tirana)")
print("     owner@learningcenter.al  (learning-center)")
print("     owner@amos.al            (amos-realestate)")
print("     owner@adriatictours.al   (adriatic-tours)")
print("     owner@ndertim.al         (ndertim-shpk)")
print("     owner@marketexpress.al   (market-express)")
print("     owner@hotelriviera.al    (hotel-riviera)")
print("   Customer   : user@user.com")

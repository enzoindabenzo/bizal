...
class ContactReplyTest(TestCase):
    ...
    def test_reply_to_nonexistent_message_returns_404(self):
        """Covers the ContactMessage.DoesNotExist branch (lines 128-129):
        a real message id, but one that doesn't belong to self.tenant, so
        IsTenantOwner passes and the view's own .get(pk=..., tenant=...)
        lookup is what fails."""
        other_tenant = make_tenant('missingreply')
        other_msg = ContactMessage.objects.create(
            tenant=other_tenant, name='Elsewhere',
            email='elsewhere@test.com', message='Not yours',
        )
        self.client.force_authenticate(user=self.owner)
        resp = self.client.post(
            f'/api/contact/admin/messages/{other_msg.pk}/reply/',
            {'reply': 'Should not go through'},
        )
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)

    def test_reply_smtp_failure_returns_502(self):
        """Covers the SMTP-failure except branch (lines 141-147): a valid
        reply that fails to send should surface as a 502 to staff, not a
        silent success or a 500."""
        self.client.force_authenticate(user=self.owner)
        with patch('contact.views.send_mail', side_effect=Exception('SMTP connection refused')):
            resp = self.client.post(
                f'/api/contact/admin/messages/{self.msg.pk}/reply/',
                {'reply': 'This will fail to send.'},
            )
        self.assertEqual(resp.status_code, status.HTTP_502_BAD_GATEWAY)
        self.assertIn('mail server error', resp.data['detail'])
        self.msg.refresh_from_db()
        self.assertIsNone(self.msg.replied_at)
...
class ContactMessageAdminMarkAsReadTests(TestCase):
    def setUp(self):
        self.superadmin = User.objects.create_user(
            email='root-contact@bizal.al', password='rootpass',
            is_superuser=True, is_staff=True,
        )
        self.client.force_login(self.superadmin)
        self.tenant = Tenant.objects.create(
            name='Contact Co', slug='contactco-admin', business_type='restaurant',
            plan='pro', is_active=True,
        )
        self.msg = ContactMessage.objects.create(
            tenant=self.tenant, name='Ana', email='ana@example.com',
            subject='Pyetje', message='Hello', is_read=False,
        )

    def test_mark_as_read_action(self):
        resp = self.client.post('/django-admin/contact/contactmessage/', {
            'action': 'mark_as_read',
...
class PlatformInquiryAdminMarkAsReadTests(TestCase):
    def setUp(self):
        self.superadmin = User.objects.create_user(
            email='root-inquiry@bizal.al', password='rootpass',
            is_superuser=True, is_staff=True,
        )
        self.client.force_login(self.superadmin)
        self.inquiry = PlatformInquiry.objects.create(
            name='Bora', email='bora@example.com', subject='Info', message='Hi', is_read=False,
        )

    def test_mark_as_read_action(self):
        resp = self.client.post('/django-admin/contact/platforminquiry/', {
            'action': 'mark_as_read',
            '_selected_action': [str(self.inquiry.pk)],
        }, follow=True)
        self.assertEqual(resp.status_code, 200)

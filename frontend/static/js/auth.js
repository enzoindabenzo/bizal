/* ═══════════════════════════════════════════════════════
   ...
   Access tokens are now stored in memory (a module-level variable)
...
// On port 8001 in local dev, TenantMiddleware resolves the tenant
...
// Single source of truth for "what's the URL of a tenant's admin panel",
// used by any page that needs to send someone there (onboarding completion,
// post-signup redirect, etc.). Centralized here instead of each caller
// re-deriving isLocal/localhost:8001/*.bizal.al itself, which previously
// handled only the exact 'localhost' hostname and silently mis-routed on
// 127.0.0.1, a staging subdomain, or any other non-'localhost' dev host.
function getTenantAdminUrl(slug) {
  const h = window.location.hostname;
...
// Expose for testing (matches the window.esc/window.BizBot pattern already
// used elsewhere in this codebase). In production this is just how any
// top-level `const` script already behaves as a browser global.
window.Auth = Auth;
